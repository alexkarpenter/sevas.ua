	...
	Lightbox = (function () {
		...
		Lightbox.prototype.build = function () {
			...
			}).append($('<div/>', 
			{
				...
				"class": 'share42initSLIDE share42style'
			}), $('<div/>', {
				"class": 'lb-closeContainer'
			}).append($('<a/>',{"class": 'lb-close'}).append($('<img/>', {
...
*/				/*$('<div/>', {'class': 'lb-fullScreen'}).appendTo('.lb-container');
		...
		/*Lightbox.prototype.start = function ($link) {
			...
			imageNumber = 0;
            var link = $(".list li");
            var length_link = $(".list li").length;
			if ($link.attr('rel') == 'lightbox') {    
             for (i = 0, _len = length_link; i < _len; i++) {  
				a = link[i];  
				this.album.push({
					link: $link.attr('href'),
          ...
          /*  arr = $('.list li');
             $.each(arr, function(i) {
            console.log(parseInt(i)+parseInt(1) + '' +$(this)[0]['className']);
           if($(this)[0]['className']!=''){
               imageNumber = parseInt(i)+parseInt(1);
           }
            })*/
	...
	*/	
		...
		Lightbox.prototype.updateDetails = function () {
			...
			if (this.album.length > 1){
				...
				console.log(this.currentImageIndex+1);}
			else
