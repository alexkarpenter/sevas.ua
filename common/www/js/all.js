    ...
    $('#check_cat input').change(function(){
        ...
        if(!state) {
            if( catalogNews[id]['chlds'].length > 0 )
            {
                ...
                {
                    $("#" + catalogNews[id]['chlds'][i]).prop("checked", false);
                }
            }
            else return;
        }
        if(state) {
            if( catalogNews[id]['prnts'].length > 0 )
            {
                ...
                {
                    $("#" + catalogNews[id]['prnts'][i]).prop("checked", true);
                }
            }
            else return;
        }
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                }else if(obj.hasError == 0)
                {
                    ...
                    $('#inpt_pus').replaceWith("<div class='user_icon'></div>"+obj.username);
...
$(document).ready(function(){
    $('#registration-data-submit-button').click(function(){
        var login = ($('#RegForm_login').val() != 'Желаемый логин')? $.trim($('#RegForm_login').val()) : '';
        var mail = ($('#RegForm_email').val() != 'Email')? $.trim($('#RegForm_email').val()) : '';
        var pass = ($('#RegForm_password').val() != 'Пароль')? $.trim($('#RegForm_password').val()) : '';
        var pass_repeat = ($('#RegForm_password_repeat').val() != 'Пароль еще раз')? $.trim($('#RegForm_password_repeat').val()) : '';


        if(login == '') $('.user_reg_error').text('Поле Login не заполнено')
        else if(isBusyLogin(login) == false) $('.user_reg_error').text('Такой логин уже используется')
        else if(mail == '') $('.user_reg_error').text('Поле Mail не заполнено')
        else if(isValidEmailAddress(mail) == false) $('.user_reg_error').text('E-mail введен не корректно ')
        else if(pass == '' || pass_repeat == '') $('.user_reg_error').text('Поле пароль не заполнено')
        else if(pass != pass_repeat) $('.user_reg_error').text('Пароли должны совпадать')
        else
        {
            sendRegForm(login, mail, pass);
            $('.user_reg_error').text('');
            $('#reg-form').css('display', 'none');
            $('#success-registration').css('display', 'block');

            $('#RegForm_login').val('Желаемый логин');
            $('#RegForm_email').val('Email');
            $('#RegForm_password').val('');
            $('#RegForm_password_repeat').val('');
        }
    });
...
//send ti registration action
function sendRegForm(login, mail, pass)
{
    $.ajax({
        url: "/user/reg",
        type: "POST",
        ...
        success: function(data){
            ...
            {
                $('#success-registration #block').text('Ошибка при отправке');
...
/*State radiobutton on user list*/
//:not(:checked)
$(document).ready(function(){
...
/*END State radiobutton on user list*/