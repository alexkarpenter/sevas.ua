...
class Comment extends CActiveRecord
{
	...
	public function validateType2()
	{
		if (!$type = $this->type)
			throw new CException('type error');
		$o = new $type;
		/* @var $commentable CommentableBehavior */
		if (!$commentable = $o->commentable)
			throw new CException('type error 2');
		$comment = Yii::createComponent($commentable->commentModelClass);
		if (get_class($comment) != get_class($this))
			throw new CException('type error 3');
	}
	...
	protected function afterSave()
	{
		...
		$host = $this->findHostModel();
		// raise update comment event
		$this->module->onUpdateComment($this, $host);
	}

	protected function beforeDelete()
	{
		$this->host = $this->findHostModel();
	...
	protected function afterDelete()
	{
		parent::afterDelete();
		// raise update comment event
		$this->module->onDeleteComment($this, $this->host);
	}
	 ...
	 */
	public function findHostModel()
	{
		// todo: rewrite module!!!
		if (!$this->getType())
			return null;
		if (!$host = Yii::createComponent($this->getType()))
			return null;
		/* @var $commentable CommentableBehavior */
		if (!$commentable = $host->commentable)
			return null;
		...
		if (!$link = $q->queryRow())
			return null;
		if (!$hostId = $link[$columnHost])
			return null;
		return $host->findByPk($hostId);
	}
