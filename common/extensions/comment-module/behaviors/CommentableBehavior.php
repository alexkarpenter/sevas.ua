...
class CommentableBehavior extends CActiveRecordBehavior
{
	...
	{
		$comment = Yii::createComponent($this->commentModelClass);
	...
	{
		$comments = Yii::createComponent($this->commentModelClass)
	...
	{
		return Yii::createComponent($this->commentModelClass)
					->count($this->getCommentCriteria());
	}

	protected function getCommentCriteria($commentTableAlias=null)
	{
	...
	private static function callCommentEvent($eventName, $event) {
		...
		if (method_exists($host, $eventName)) {
			call_user_method($eventName, $host, $event);
		}
	}
