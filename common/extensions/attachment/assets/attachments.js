(function($){
	...
	Attachment.prototype.render = function() {
		...
		var h = tmpl(this.data.id+'_attachments_templ', {data:this.data});
		this.cont.html(h);
	...
	Attachment.prototype.upload = function(chooser) {
		...
		$.ajaxFileUpload({
				...
				success: function (data, status)
				{
					...
					this.render();
					/*
					if (data.code == 0 && data.urlPreview) {
						self.url(data.url);
						self.urlPreview(data.urlPreview);
						self.state('loaded');
					} else {
						self.loadErrorCode = data.code;
						self.upload_max_filesize = data.upload_max_filesize;
						self.state('error');
					}
					model.afterRender();
					*/
